...
namespace AD.Server.Controllers
{
    ...
    {
        private readonly ICurrencyServices _entityServices;
            ...
            try
            {
                ...
                if (String.IsNullOrWhiteSpace(paginationSearchDTO.SearchText))
                    ...
                    list = await _entityRepository.TableNoTracking
                        .Where(x => x.Code.ToLower().Contains(paginationSearchDTO.SearchText.ToLower()) && x.Status == Status.Online)
        ...
        [HttpPost]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
        public async Task<ActionResult> Post(Currency entity)
        {
            #region Start the watch   
            var watch = new Stopwatch();
            watch.Start();
        ...
        [HttpPut]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
        public async Task<ActionResult> Put(Currency entity)
        {
            #region Start the watch   
            var watch = new Stopwatch();
            watch.Start();
