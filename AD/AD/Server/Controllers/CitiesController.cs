...
namespace AD.Server.Controllers
{
    ...
    {
        private readonly ICityServices _entityServices;
        ...
        public async Task<ActionResult> Get([FromQuery] PaginationSearchDTO paginationSearchDTO)
        {
            try
            {
                ...
                if (String.IsNullOrWhiteSpace(paginationSearchDTO.SearchText))
                    list = await _entityRepository.TableNoTracking.Where(x => x.Status == Status.Online).Include(x => x.Country).OrderByDescending(x => x.Id).ToListAsync();
        ...
        [HttpPut]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
        public async Task<ActionResult> Put(EditCityVMRequest entity)
        {
            #region Start the watch   
            var watch = new Stopwatch();
            watch.Start();
